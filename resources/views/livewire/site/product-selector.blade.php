<div>
    <div class="mb-3">
        ...
        <var class=" h4"> سعر المنتج : {{ $product->price_after_discount }} {{__('site.SAR')}}  </var>
        <span class="text-muted old-price"> <del>{{ $product->price }}</del> {{__('site.SAR')}}</span>
        @else
        <var class=" h4"> سعر المنتج : {{ $product->price }} {{__('site.SAR')}}  </var>
        @endif
    </div>
    <dl class="row">
        <dt class="col-sm-3 "> الربح من اجمالي السعر : </dt>
        <dd class="col-sm-9 h4" style='color:#53cc05' > {{ $product->marketer_price }} {{__('site.SAR')}} </dd>
