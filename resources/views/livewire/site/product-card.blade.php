    ...
    <div href="{{ $product->url() }}" class="card card-sm card-product-grid">
     ...
     <figcaption class="info-wrap">
        ...
        <div class="price-wrap">
           @if ($product->hasDiscount())
           <span class="price"> {{ $product->price_after_discount }}  {{__('site.SAR')}} </span>
           <del class="price-old"> {{ $product->price }}  {{__('site.SAR')}} </del>
           @else
           <span class="price"> {{ $product->price }} {{__('site.SAR')}} </span>
           @endif
       </div>
   </figcaption>
</div>
