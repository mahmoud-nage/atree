    ...
    <form action="{{ route('withdrawals.store') }}" method='POST'>
        ...
        <div class="form-group">
            <label class="custom-control custom-radio custom-control-inline">
                <input class="custom-control-input" wire:model='type' type="radio" name="payment_method"
                       value="1" {{ old('payment_method') == 1 ? 'checked' : '' }} >
                <span class="custom-control-label"> محفظه الكتورنيه </span>
            </label>
            <label class="custom-control custom-radio custom-control-inline">
                <input class="custom-control-input" wire:model='type' type="radio" name="payment_method"
                       value="2" {{ old('payment_method') == 1 ? 'checked' : '' }} >
        ...
        @if ($type == 2 )
            @if($banks)
                <div class="col-md-12">
                    <div class="form-group">
                        <label for=""> {{__('site.banks')}} <span class='text-danger'> * </span> </label>
                        <select class='form-control' name="bank_account_id">
                            <option>{{__('site.banks')}}</option>
