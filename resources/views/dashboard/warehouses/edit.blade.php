...
@endphp
@section('page_title')
{{ trans('warehouses.edit_warehouse_details') }}
@endsection
...
<span class="breadcrumb-item active"> @lang('warehouses.edit_warehouse_details') </span>
@endsection
	...
	<div class="col-md-12">
		<div class="card">
			<div class="card-header bg-primary text-white header-elements-sm-inline" >
				<h5 class="card-title"> @lang('warehouses.edit_warehouse_details') </h5>
			...
			<form action="{{ route('dashboard.warehouses.update' , ['warehouse' => $warehouse->id ]) }}" method='POST' enctype="multipart/form-data" > 
				@csrf
				@method('PATCH')
				<div class="card-body">
					...
					<fieldset class="mb-3">
						<legend class="text-uppercase font-size-sm font-weight-bold"> @lang('warehouses.warehouse_details') </legend>
								...
								<div  class='mb-2' >
									...
									<select name="manger_id" class='form-control' id="">
										@foreach ($users as $user)
											<option value="{{ $user->id }}" {{ $warehouse->manger_id == $user->id ? 'selected="selected"' : ''  }} > {{ $user->name }} </option>
										@endforeach
									</select>
									@error('manger_id')
									<p  class='text-danger' >  {{ $message }} </p>
							...
							@foreach ($governorates as $governorate)
								...
								<div  class='mb-2' >
									...
									<div class="custom-control custom-switch mb-2">
										...
										id="sc_ls_c{{ $governorate->id }}" {{ in_array($governorate->id, $warehouse_governorates) ? 'checked' : '' }} >
